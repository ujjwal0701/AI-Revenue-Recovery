      ...
      <div className="flex min-h-screen">
        ...
        <aside className="hidden w-64 shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
          ...
          <nav className="flex-1 space-y-1 p-4">
            ...
            <button className="flex w-full items-center gap-3 rounded-xl bg-slate-950 px-3 py-2.5 text-left text-sm font-medium text-white">
              <Icon name="trend" size={18} />
              Overview
            </button>

            <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-500 hover:bg-slate-50">
      ...
      onClick={async () => {
        try {
          ...
          if (response.ok && data.success) {
            window.location.href = `\/recover\/${payment.id}`;
          } else {
  ...
  {[
    ...
    [
      payments
        .reduce(
          (total, payment) =>
            total + (payment.recoveryAttempts?.length || 0),
          0
        )
        .toString(),
      "Recovery attempts",
